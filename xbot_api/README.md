...
```
GET /api/game/new?mode=map&client_request_id=abc123
```

**Parameters**:
- `mode` (optional): Game mode (`map`, `flag`, or `cap`). Defaults to `map`.
- `client_request_id` (recommended): A unique identifier generated by the client to prevent duplicate or inconsistent responses. This is crucial for preventing UI flickering issues when the same request may be processed multiple times.

**Headers**:
- `X-API-Key`: Your API key

**Client Request ID Deduplication**:
- The API uses the `client_request_id` to ensure that identical requests always receive identical responses
...
# Get a new game with client request ID for deduplication
def get_new_game(mode="map", client_request_id=None):
    ...
    response = requests.get(f"{API_BASE_URL}/game/new", 
                           params={"mode": mode, "client_request_id": client_request_id},
                           headers=HEADERS)
    result = response.json()
    # Store the client request ID with the game data for future reference
